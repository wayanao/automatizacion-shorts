...
def download_background_videos(keywords: list[str], output_dir: Path, min_clips: int = 5) -> list[Path]:
    ...
    search_queries = list(keywords) if keywords else [
        "peaceful sunrise nature",
        "calm ocean waves",
        "person praying sunset",
    ]
    if len(search_queries) < 3:
        search_queries.extend(["forest light", "rain window", "quiet mountain landscape"])
    ...
    if not all_videos:
        all_videos = _search_videos_for_query("peaceful nature prayer", per_page=10)
