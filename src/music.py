"""Elige una pista de musica suave de fondo (CC BY 3.0, Kevin MacLeod / incompetech.com)."""
import random
from src import config

# nombre de archivo -> metadatos de la pista, incluye categoria para elegir el ambiente adecuado
# Agrega tus propios mp3 de estilo "peaceful" en assets/music/ con estos nombres y actualiza
...
TRACKS = {
    ...
    "track1.mp3": {"title": "Wallpaper", "category": "peaceful"},
    ...
    "track5.mp3": {"title": "Deliberate Thought", "category": "peaceful"},
}
...
def pick_background_music(category: str = "peaceful") -> tuple[str, str] | tuple[None, None]:
    """Devuelve (ruta_absoluta_al_mp3, texto_de_atribucion) o (None, None) si no hay pistas.

    Prioriza pistas de la categoria pedida (por defecto "peaceful"); si no hay ninguna disponible
    en disco, cae de vuelta a cualquier pista local.
    """
    ...
    available = _available(category) or _available("general") or _available("news")
    if not available:
        return None, None
